...
async def test_a_scripted_chart_passes_the_check_and_reaches_the_transcript(
    ...
    # The chat model, the chart sub-agent, the query sub-agent and the follow-up step.
    assert scripts.resolved == ["fast", "fast", "fast", "fast"]
    ...
    assert [p["type"] for p in parts] == ["tool-chart", "reasoning", "text", "data-followups"]
