...
from .conftest import Chat, Scripts, is_followup_request, new_conversation, script
...
def scripted_categorizer(guesses: dict[str, tuple[str, str | None, float]] = GUESSES):
    ...
    The follow-up step and, when a test asks a question, the query sub-agent share this slot,
    so both are recognized and answered here too. `prompts` records every categorizer prompt,
    which is how a test sees whether a merchant ever reached the model at all.
    """
    prompts: list[str] = []
    queries: list[str] = []
...
async def test_answering_a_question_card_creates_rules_and_recategorizes(
    ...
    # the text. A reload shows what the stream showed.
...
async def test_a_card_the_user_skips_leaves_the_rows_alone(
    ...
    reloaded = (await client.get(f"/api/conversations/{conversation_id}")).json()
    parts = reloaded["messages"][1]["parts"]
    assert [part["type"] for part in parts] == ["text", "tool-ask_user", "text"]
    assert parts[1]["output"] == {"answers": []}
