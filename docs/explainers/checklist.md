...
| Start, stop and resume: switching between conversations | done | `frontend/src/components/app-sidebar.tsx:AppSidebar`; a running turn survives a switch (`src/finquery/api/running.py:RunningTurn`, `src/finquery/api/chat.py:reattach`) | `tests/test_background_turns.py` (8 tests); ticket 33 | 01 |
...
| At least two fine-tuned models (LoRA or QLoRA) | **done** (four QLoRA adapters trained 2026-09-06/07, scored against vanilla on the frozen benchmark; the E4B query adapter ships, 62 to 78 percent figure match on 459 questions; the E4B chart adapter does not, 38 to 36 on two runs; the 12B adapters are dropped, query 78 to 82 inside the noise and chart 62 to 50) | training `training/cluster/train.py`, conversion `training/cluster/convert.sh`, attach `src/finquery/local/adapters.py:AdapterRegistry`, `src/finquery/local/runtime.py:LocalStack` (`with_adapter`), bench `--adapter` in `bench/finquery_bench/models.py:local_target` | `bench/results/20260907-adapters-compare.md`; `tests/test_training_cluster.py`, `tests/test_local_provider.py` | 04 |
...
| Video demo: at least one real-world use case that benefited from the electives | **not started** (use case identified) | unknown merchant: web lookup, categorizer, memory; demo steps 23 to 25 and 17 to 18 | tickets 14, 17 | 05 |
...
| Repeated self-controlled web search: finds and visits pages | **done** (claimed) | `src/finquery/weblookup/loop.py:run_loop` (search, fetch, finish; 4 searches, 3 page reads), `src/finquery/weblookup/client.py:HttpWebClient` (fetch and HTML to text), `src/finquery/weblookup/scrub.py:scrub`, `src/finquery/weblookup/store.py:OutboundJournal`, `src/finquery/agent.py:lookup_merchant`, `src/finquery/categorize/pipeline.py:_look_up` | `tests/test_web_lookup.py` (12 plus 2); ticket 14 (Nordsee two searches; ten log rows, tokens only); demo step 24 (one search, one page read, 53 s) | 15 |
| Preference optimisation: a choice of generated texts, picked by the user | built, **removed**, not claimed (ticket 59, 2026-09-06) | nothing in the tree: the thumbs, the Regenerate pair, the A/B and the Feedback page were removed. In the history: `git show 88c1f07^:src/finquery/api/preferences.py`, `git show c988577^:frontend/src/components/feedback.tsx` | tickets 15, 22, 30 built it; ticket 59 removed it, with the reason in explainer 14 | 14 |
| Preference optimisation: code to train a LoRA or QLoRA on the choice data | built, **removed**, not claimed (ticket 59, 2026-09-06) | nothing in the tree: `training/preference/` (the DPO export and the TRL script) was removed with the feature it read from. In the history: `git log --diff-filter=D --name-only -- training/preference` | it was never run: one query pair and no chart pair on the only database it was pointed at (ticket 15) | 14 |
...
- Required: 11 bullets done, 2 not started (the two halves of the
  video demo).
- Electives claimed (pair, four required): context management done; sub-agent deployment done
  ...
  ticket 58) in place of preference optimisation, which was built and then removed the same day
  (ticket 59, explainer 14).
