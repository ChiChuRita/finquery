...
| Multimodal ingestion (images, PDF and more; a vision-language model) | built, not claimed as an elective | `src/finquery/extract/statement.py:extract_statement` (PDF text layer and rendered pages), `src/finquery/extract/subagent.py:read_statement_image`, `read_bill` (vision on the fast slot), `src/finquery/extract/bill.py:bill_outcome`; CSV in `src/finquery/ingest/csv_reader.py:parse` | `tests/test_extraction.py`; ticket 11 (15 pages, 433 of 433, reconciled); ticket 42 (twenty receipts). Not covered: audio, Word, Excel | 08 |
...
- Electives claimed (pair, four required): context management done; sub-agent deployment done
  (extra credit not claimed); web search done; multimodal ingestion in place of preference
  optimisation, which was built and then removed on 2026-09-06 (ticket 59, explainer 14).
- Built but not claimed: multimodal ingestion (PDF, scans, receipt photos through a vision
  model); sandboxed execution of generated chart code (JavaScript, not Python).
