...
| At least two fine-tuned models (LoRA or QLoRA) | **done** (four QLoRA adapters trained 2026-09-06/07, scored against vanilla on the frozen benchmark; the E4B query adapter ships, 62 to 78 percent figure match on 459 questions; the E4B chart adapter does not, 38 to 36 with drawn 85 to 72; the 12B query adapter 78 to 82 is inside the noise; 12B chart pending) | training `training/cluster/train.py`, conversion `training/cluster/convert.sh`, attach `src/finquery/local/adapters.py:AdapterRegistry`, `src/finquery/local/runtime.py:LocalStack` (`with_adapter`), bench `--adapter` in `bench/finquery_bench/models.py:local_target` | `bench/results/20260907-adapters-compare.md`; `tests/test_training_cluster.py`, `tests/test_local_provider.py` | 04 |
