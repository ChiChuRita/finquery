...
| Switch the LLM: at least two loadable models | done | `src/finquery/providers.py:build_resolver`, `ModelSlot`; `src/finquery/local/runtime.py:LocalStack` (both resident); `frontend/src/components/model-picker.tsx:ModelPicker`; the slot per conversation and per turn | `tests/test_conversations.py` (slot switch mid-conversation, every turn carries its slot); `tests/test_local_provider.py`; `finquery-check` all eight checks (ticket 23) | 02 |
...
- Electives claimed (pair, four required): context management done; sub-agent deployment done
  (extra credit not claimed); web search done; preference optimisation done on the choice half
  and partial on the training half (script written, not run).
