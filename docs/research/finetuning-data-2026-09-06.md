...
| 2 | **Gemma 4 E4B per-layer embeddings: checked, not a blocker** | Issue 22243 (https://github.com/ggml-org/llama.cpp/issues/22243) was closed on 2026-04-23 by a maintainer: the per-layer embeddings are implemented in `src/models/gemma4-iswa.cpp`, and both embedding tensors are kept at Q4_K or better by the quantizer. Our build (llama-cpp-python 0.3.35) carries the `embedding_length_per_layer_input` and `per_layer_embd_out` symbols. The earlier draft of this row read the open report, not the answer; verified by the coordinator on 2026-09-06 |
...
check llama.cpp issue 21125 before any Qwen adapter is converted; issue 22243 was verified closed and does not affect E4B numbers.
