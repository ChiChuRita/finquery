...
- Ticket 13's memories go through `assemble` (it takes the block selected for the turn and joins
  it to the note), and their count rides in the same data part, where the badge already had a
  slot for it.
...
- A compressed prompt has a floor: the system prompt plus the summary plus the last six turns.
  Every line added to the system prompt raises that floor, and a budget below it cannot be met by
  compressing. Harmless at 32768 and at the local 16384; it is why the small budget in
  `tests/test_context.py` had to grow when the system prompt did.
