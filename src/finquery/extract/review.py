...
async def apply_review(
    ...
    *,
    ...
    # A statement PDF meets the duplicate step like any other import, and this applier answers
    # with a line rather than a card, so the next card has to be asked for by name.
    if duplicates.tally(session, profile_id, import_id=committed.id).pending:
        said.append("Call `review_duplicates` for the first card about them.")
    if report.error:
