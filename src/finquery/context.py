...
Ticket 13 selects memories into the same assembly; `assemble` is the seam it extends.
"""
...
class Assembly:
    ...
    """The rolling summary as a system-level note, appended to the agent's own instructions.
    `None` when nothing has been compressed yet."""
...
def assemble(turns: Sequence[TurnMessages], summary: str | None, summary_through: int) -> Assembly:
    """Build one turn's prompt: the summary as a note, then the turns after the marker."""
    ...
    return Assembly(
        ...
        tokens=estimate_tokens(history, "\n".join(filter(None, [SYSTEM_PROMPT, note]))),
    )
