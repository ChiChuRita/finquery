...
One kind exists today, `category_rule`: every answer becomes a category rule for that
merchant. A card says which kind it is through `AskUser.apply`, so tickets 08 (mapping
confirmation) and 10 (duplicate decisions) add an entry to `APPLIERS` and touch nothing on the
...
def _apply(
    ...
    if applier is None:  # pragma: no cover - every declared kind has an applier
        return None
