...
every local model whose weights are on disk, one at a time, which is also how the app runs them
(ticket 68: one model is loaded at a time), so it is the slowest thing in the project and the
fastest way to find out that the local setup is broken.

There is no memory figure here. The process RSS reads as 0.1 GB with a 13 GB GGUF mmapped
(`bench/results/20260907-local-tokens-per-second.md`), so the old pair report called a pair
"ok" that could not decode; a wrong number is worse than none.
"""
...
async def check_slot(stack: LocalStack, spec: ModelSpec) -> SlotReport:
    """Load one local model, in place of whatever was loaded, and run all four checks on it."""
...
def checkable(stack: LocalStack) -> tuple[ModelSpec, ...]:
    """Gemma 4 E4B, plus every local chat model whose files are on disk.
