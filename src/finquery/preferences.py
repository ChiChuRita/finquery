...
MUTATING_TOOLS = ("propose_changeset", "apply_simple_edit", "set_rule", "ask_user")
"""Tools a rerun must not repeat: they write, or they wait for a human.

A turn that used one of them gets no answer A/B. `query` and `chart` are read-only, so a second
answer may call them again.
"""
