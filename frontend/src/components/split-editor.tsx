...
export function SplitEditor({
  ...
  const { data: children, isPending } = useQuery(splitsQuery(transaction.id))
  ...
  const save = useMutation({
    mutationFn: (children: SplitChild[]) => saveSplits(transaction.id, children),
    onSuccess: async () => {
      setDraft(null)
      setError(null)
      await queryClient.invalidateQueries(splitsQuery(transaction.id))
      onChanged()
    },
