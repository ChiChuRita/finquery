...
function Row({ record }: { record: PreferenceRecord }) {
  ...
  return (
    <li className="flex items-start gap-3 px-4 py-3">
      ...
      <span className={cn('mt-0.5 flex shrink-0 items-center gap-1.5 text-xs', rating.className)}>
...
export function FeedbackPage() {
  ...
  return (
    <DocumentPage
      description="Every thumb, every chart you picked and every second answer you preferred is kept as a preference record: the prompt, the output you kept and the one you refused. The export is the training set for the two fine-tuned sub-agents."
      title="Feedback"
    >
        ...
        {!records || records.length === 0 ? (
          <Empty className="border">
            <EmptyHeader>
              ...
              <EmptyTitle>Nothing rated yet</EmptyTitle>
              <EmptyDescription>
                Use the thumbs under an answer or a chart, regenerate a chart and pick the better one, and the
                records appear here.
              </EmptyDescription>
            </EmptyHeader>
          </Empty>
