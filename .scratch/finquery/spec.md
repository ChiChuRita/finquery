...
- 2026-09-06: The default dashboard follows the research note `docs/research/charts-and-dashboard-2026-09-06.md` (ticket 51): four tiles with deltas against last month and the six-month average, then six cards, all from guarded SQL with the arithmetic for deltas and folds done on the returned rows: spending per month as bars (the line goes), income against spending (stays), top categories as ranked bars (the doughnut leaves the default set, stays a chat shape), this month against last month by category, regular payments (a heuristic named as such), top merchants for the range (stays), plus month pacing as a two-series line once ticket 50 is in. The chart sub-agent gains no new shape families beyond ticket 50; it gains worked examples for period comparison bars, diverging net bars and a cumulative two-year area, and one small contract change, a reference line (`ruleY`) whose value derives from the rows (ticket 52).
- 2026-09-06: The chat model is chosen from a catalog that mixes providers (ticket 54): Qwen3.5 9B (local), Qwen3.5 9B (cloud), Gemma 4 12B (local), Gemma 4 26B (cloud). The provider switch stops being the only way to reach a provider: both are usable at once when their prerequisites are met (weights on disk, an API key), and `FINQUERY_PROVIDER` only decides the default entry of a new conversation. Sub-agents keep running on the fast slot of the provider the chat entry belongs to: Gemma 4 E4B locally (resident, adapters attach there), the hosted fast model in the cloud. Locally the two chat models share one seat and are swapped on demand; E4B stays resident.
- 2026-09-06: Benchmarks and fine-tuning run on the HPI cluster over the user's VPN, never on the laptop and never against the OpenRouter key; hosted models are not benchmarked (tickets 56 and 57). The candidates are the local models the product can ship: Gemma 4 E4B, Qwen3.5 9B, Gemma 4 12B, run on the cluster with llama-cpp and the same GGUFs as the laptop so a cluster number equals a laptop number. The unit of the benchmark is the sub-agent path (run_query with the check pass, run_chart with the self-check) because the adapters attach there; a small end-to-end subset also runs the chat agent's turn that writes the tool request. Decision rule for the shipped pair: accuracy first, speed as a tie-breaker within about five points, and the two models must fit together in the laptop's memory. The benchmark's held-out third is the fine-tuning validation set and grows with the generated data under the same split rule.
- 2026-09-06: The four claimed electives change (tickets 58 and 59): multimodal ingestion replaces preference optimisation via human feedback. Multimodal ingestion is claimed with images and PDFs through the two local vision-language models under the extraction guards, plus XLSX and DOCX imports through parsing libraries; audio is out of scope and said so. The preference feature (thumbs, Regenerate pair, A/B, Feedback page, DPO export and training scripts) is removed from the product, not merely unclaimed. Claimed set: intelligent context management, sub-agent deployment, multimodal ingestion, self-controlled web search.
- 2026-09-06: Typing answers a card (ticket 60). While any card the agent asks with is open (Question cards of every kind and the changeset preview), the composer is the place to type the answer: a message that names an option or a row plus an option is matched deterministically with no model call, anything else goes to the chat agent as the card's free-text answer and is mapped to rows and options, and an unrelated message is answered while the card stays open. Answered rows apply and the rest stay open. The composer shows a hint and the open card's single-choice options as chips; the cards keep their buttons and lose their per-row free text fields.
- 2026-09-06: The shipped local pair, decided on the cluster benchmark (`bench/results/20260906-cluster-compare.md`, ticket 61): Gemma 4 12B is the default chat model and, for the demo, the model every sub-agent runs on; Gemma 4 E4B stays resident as the fast slot, is the adapter target, and is the fallback for any sub-agent role that is switched to it. Which model each sub-agent role uses is one setting per role. Qwen3.5 9B stays in the catalog as an alternative chat model. Hosted development and agent verification run Gemma 4 26B A4B on both cloud slots, the same family and wire format as the local pair. This supersedes the 2026-09-04 amendment that made Qwen3.5 9B the quality slot.
