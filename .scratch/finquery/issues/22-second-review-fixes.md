...
Merged into main on 2026-09-05. Beyond the two mechanical hunks: the extraction review card import in ingest/chat_import.py shadowed the categorization review_card (both named review_card, one from ticket 11, one wired by ticket 22), which broke every chat CSV import with "review_card() takes 1 positional argument but 2 were given"; the extraction one is now imported as extraction_review_card. The test context budget was re-measured on the merged prompt (2564 tokens) and set to 4000.
