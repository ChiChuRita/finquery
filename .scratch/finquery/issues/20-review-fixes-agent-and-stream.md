...
Merged into main on 2026-09-04 on top of 01 to 09, 12 to 16, 18 and 19. What changed in the
merge:

- **Ticket 08's two cards now declare an apply kind, and that was the one real integration
  question.** This ticket's dispatcher treats a card that declares nothing as a categorization
  card, which was safe when the only cards were the seeded review turn and `review_batch`. With
  ticket 08 merged there are two more: the mapping confirmation from `import_file` and the
  transaction preview from `extract_transaction`, whose answers are Confirm and Discard and
  whose effect is a tool the model calls itself. Left alone they would have gone to
  `apply_category_rules`, which would have offered "add" and "confirm" to `set_rule` as category
  names. `ApplyKind` therefore gained `mapping_confirmation` and `transaction_draft`, both
  deliberately absent from `APPLIERS`, so `_apply` returns None and the answers reach the model
  untouched, which is exactly ticket 08's behaviour. The `# pragma: no cover` on that branch is
  gone because it is now the normal path for two of the four card kinds, and the module
  docstrings of `answers.py` and `ask_user.py` say why a kind with no applier exists.
  `tests/test_chat_import.py` pins both halves: the card carries `{"kind":
  "transaction_draft"}` and the answered card's `applied` is None. Removing either
  `apply=AskApply(...)` fails that test.
  Ticket 10 adds its duplicate kind to `ApplyKind` *and* to `APPLIERS`, since the server does
  apply those.
- `chat-view.tsx` is the union of five branches' work: eleven tool renderers (query, ask_user,
  import_file, extract_transaction, add_transaction, set_rule, review_batch, remember,
  propose_changeset/apply_simple_edit, chart, lookup_merchant), ticket 08's `progress` and
  ticket 15's `ratings` as props of `TranscriptMessage`, and this ticket's `MemoryToolStep`.
- `rule-tool.tsx` is this ticket's version whole (collapsible `Tool` steps with a body).
  `components/tool-step.tsx` stays because ticket 08's `import-tool.tsx` uses it.
- `README.md` names both `weblookup/` and `answers.py` in the module list.
- **The test context budget went from 3000 to 3400 tokens.** Measured rather than guessed this
  time: the system prompt is 2110 tokens with every block on it, and the floor a compressed
  prompt cannot go below is 3018, so 3400 leaves about 11 percent. The comment now says which
  of the two compression conditions the constant is actually for (the floor, not the 60 percent
  trigger, which the system prompt alone passes on turn one).
- `uv run pytest` is 123 passed, 1 skipped (`tests/test_answers.py` and this ticket's additions
  bring 117 to 123). `npx tsc --noEmit`, `npm run build` and `oxlint` are clean.
- Verified on OpenRouter on port 8077 with a throwaway database and the Sparkasse preset (433
  rows, 384 by the dictionary, 24 by the model, 25 Needs review). The seeded review conversation
  opened with the four-merchant card and a dash in the header badge, not NaN. Three answers
  (Jonas Keller, Max Schulz, Anna Weber, all Dining) were in the database within three seconds
  of pressing Send, before the model had said anything, Lea Hoffmann stayed Needs review, and
  the card's `applied` line named all four ("... Left for later: Lea Hoffmann (via PayPal)")
  with the model only summarizing it. Then "I paid 12 EUR cash for lunch today" in the same
  conversation previewed the draft card, Confirm wrote the booking (-1200, Cash, source manual)
  and **that card's `applied` came back None**, which is the integration above working. The
  answer read "Added the 12,00 EUR lunch payment to Cash." (German money format), and the whole
  conversation payload contains no `<turn|>`. Screenshots: /tmp/finquery-merge20/.
- Harness note on top of this ticket's own two: `agent-browser type` after a `click` did not
  reach the React composer in this session either. What worked was `eval` with the native
  `HTMLTextAreaElement.prototype.value` setter plus an `input` event, then clicking Submit.
