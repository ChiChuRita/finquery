...
### Merged into main, 2026-09-05

Merged after tickets 25, 26 and 28. Eight files conflicted; the rule was 26's structure and
tokens with 27's behaviour, copy and names, and every conflict resolved to a union rather than
to one side:

- `changeset-card`: 26's en dash for an empty cell and its comment, 27's German dates and euro
  amounts through the same `display`.
- `transactions-table`: 27's "Enriched title" header tooltip inside 26's `minmax(7rem, …)`
  column and truncating header row.
- `taxonomy-card`: 26's `Button size="xs"` pills, dashed subcategory button and `focus-ring`
  on the category name, carrying 27's three aria labels. `RenameInput` (blur proposes, `done`
  ref, Escape) came through untouched.
- `models-card`: 26's `Badge` with 27's `local` prop and `state`, so a hosted slot reads
  "hosted" on a success badge rather than "resident".
- `memory-page`, `feedback-page`, `transactions-page`: 26's `DocumentPage`, `PageBar`, `Empty`
  and `text-2xs` with 27's New chat action moved into `EmptyContent` (the shape the Imports
  page already had), 27's "Try fewer of them, or start again from every booking." plus its
  Clear the filters button, the `pushed` ref on the search box, and the dimmed rather than
  hidden Memory row actions.

**The budget was re-measured on the merged prompt, not picked.** The system prompt is 2792
tokens (27 measured 2765, main 2560) and the floor a compressed prompt cannot go below is 3710
(27 measured 3673). `BUDGET = 4400` leaves about 19 percent of headroom; both numbers are in
the comment in `tests/test_context.py`. Both sides had raised it on their own measurement, 4000
and 4500, and neither number was right for the merge.

`uv run pytest` is 187 passed, 1 skipped (the opt-in local smoke suite; the private fixtures are
present on this machine, so those tests ran). `npx tsc --noEmit`, `npm run build` and `oxlint`
are clean, 34 warnings, none in a touched file and none new. Verified in a browser on port 8096
against a throwaway database, walked through onboarding with the sample year: the Question card
answered as one card and one sentence with no table transcribed into the answer, each turn with
a tool drew exactly one thinking panel, the Memory actions are visible, the Transactions filters,
inline edit on click-away and split editor work, a category rename saved on blur into the preview
dialog with German dates, the changeset card carried both of its status notes, and both themes at
1024 and 1440 read correctly. Zero console messages.
